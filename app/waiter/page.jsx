...
export default function WaiterPage() {
  const { orders, updateOrderStatus, markTableAsCompleted, loading } = useOrders()
  const [refreshing, setRefreshing] = useState(false)
  const [selectedTable, setSelectedTable] = useState("all")
  const [activeTab, setActiveTab] = useState("ready")

  // Filter ready orders based on selected table
  const readyOrders =
    selectedTable === "all"
      ? orders.filter((order) => order.status === "ready")
      : orders.filter((order) => order.status === "ready" && order.tableId === Number.parseInt(selectedTable, 10))

  // Get unique table IDs with active (non-completed) orders
  const tableIds = [
    ...new Set(orders.filter((order) => order.status !== "completed").map((order) => order.tableId)),
  ].sort((a, b) => a - b)

  // Group orders by table for the accounts section
  ...
  // Handle closing a table (marking all orders as completed)
      ...
      <Tabs defaultValue="ready" value={activeTab} onValueChange={(value) => setActiveTab(value)}>
        ...
        <TabsContent value="ready" className="mt-0">
          <h2 className="text-lg font-medium mb-4">
            {selectedTable === "all" ? "All Orders Ready for Delivery" : `Orders Ready for Table ${selectedTable}`}
          </h2>
        ...
        <TabsContent value="accounts" className="mt-0">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {tableAccounts
                .filter((account) => selectedTable === "all" || account.tableId === Number.parseInt(selectedTable, 10))
                .map((account) => (
                  <TableAccountCard
                    key={account.tableId}
