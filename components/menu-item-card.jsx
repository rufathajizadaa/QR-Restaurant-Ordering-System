...
export default function MenuItemCard({ item, tableId }) {
  ...
  useEffect(() => {
    if (isDialogOpen && item.ingredients) {
      setIncludedIngredients(Array.isArray(item.ingredients) ? [...item.ingredients] : [])
    }
  }, [isDialogOpen, item.ingredients])
  ...
  const handleAddToCartWithCustomization = () => {
    ...
    const removedIngredients = item.ingredients
      ? Array.isArray(item.ingredients)
        ? item.ingredients.filter((ing) => !includedIngredients.includes(ing))
        : []
      : []

    // Add the item to cart with customization info
    addToCart(item, quantity, removedIngredients)
    setQuantity(1)
  ...
  const handleIngredientToggle = (ingredient, isChecked) => {
    if (isChecked) {
      ...
      // Remove ingredient from included list
